<template>
    ...
    <template v-slot:heading>
        <div class="display-2 font-weight-light">{{ categorie_id }} - PouleD - Matchs</div>
    </template>
...
<base-material-card color="colorPD" class="px-0"  >
    <template v-slot:heading>
        <div class="display-2 font-weight-light">{{ categorie_id }} - PouleD - Classement</div>
    </template>
    <v-card-text class="px-0" >
    ...
    <v-row>
        <v-col>
            <base-material-card color="colorPE" class="px-0"  >
                <template v-slot:heading>
                    <div class="display-2 font-weight-light">{{ categorie_id }} - PouleE - Matchs</div>
                </template>
                <v-card-text class="px-0" >
...
<base-material-card color="colorPE" class="px-0"  >
    <template v-slot:heading>
        <div class="display-2 font-weight-light">{{ categorie_id }} - PouleE - Classement</div>
    </template>
    <v-card-text class="px-0" >
...
<v-tab-item>
<v-container id="poulec-match" fluid tag="section">
    <v-row>
        <v-col>
            <base-material-card color="colorPF" class="px-0"  >
                <template v-slot:heading>
                    <div class="display-2 font-weight-light">{{ categorie_id }} - PouleF - Matchs</div>
                </template>
                <v-card-text class="px-0" >
                    <v-data-table :headers="headersMatch" :items="lesmatchsC" hide-default-footer class="px-0" mobile-breakpoint="350">
                        <template v-slot:[`item.id`]="{ item }">
                        <h6 class="display-1 mb-1 black--text">{{ item.id}}</h6>
                        <h6 class="display-1 mb-1 grey--text">{{ item.salle}}</h6>
                        <h6 class="display-1 mb-1 grey--text">{{ item.heureDebut}}</h6>
...
<base-material-card color="colorPF" class="px-0"  >
    <template v-slot:heading>
        <div class="display-2 font-weight-light">{{ categorie_id }} - PouleF - Classement</div>
    </template>
    <v-card-text class="px-0" >
        <v-data-table :headers="headersClassement" :items="leclassementC" hide-default-footer class="px-0" mobile-breakpoint="350">
            <template v-slot:[`item.displayEqui.nom`]="{ item }">
            <v-chip class="ma-2" label>
                <v-avatar left>
                    <v-img v-bind:src="'img/fanion/'+item.displayEqui.fanion" :alt="item.displayEqui.nomCourt"/>
...
export default {
    data() {
      return {
        ...
        urlPoule: process.env.BASE_URL + "datas/matchs_poules_seniors.json",
        lesmatchsA: [],
        leclassementA: [],
        lesmatchsB: [],
        leclassementB: [],
    ...
    methods: {
        ...
        loadDataOds() {
            ...
            axios
                .get(urlPoule)
                .then(response => {
                  this.lesmatchsA = response.data.lesmatchs.filter(function (entry){return entry.categorie_id===categorie_id;}).filter(function (entry){return entry.poule==="PD";})
                  this.lesmatchsB = response.data.lesmatchs.filter(function (entry){return entry.categorie_id===categorie_id;}).filter(function (entry){return entry.poule==="PE";})
                  this.lesmatchsC = response.data.lesmatchs.filter(function (entry){return entry.categorie_id===categorie_id;}).filter(function (entry){return entry.poule==="PF";})
                  this.leclassementA = response.data.lesclassements.filter(function (entry){return entry.categorie_id===categorie_id && entry.poule==="PD" ;})[0].leclassement
                  this.leclassementB = response.data.lesclassements.filter(function (entry){return entry.categorie_id===categorie_id && entry.poule==="PE" ;})[0].leclassement
                  this.leclassementC = response.data.lesclassements.filter(function (entry){return entry.categorie_id===categorie_id && entry.poule==="PF" ;})[0].leclassement
                  ...
                  for (var l in this.lesmatchsC ) {
                    this.lesmatchsC[l].score = this.lesmatchsC[l].equipeDom.but+" - "+this.lesmatchsC[l].equipeExt.but
                    this.lesmatchsC[l].displayDom = this.lesequipeskey[this.lesmatchsC[l].equipeDom.id]
                    this.lesmatchsC[l].displayExt = this.lesequipeskey[this.lesmatchsC[l].equipeExt.id]
                  }

                  for (var x in this.leclassementC ) {
                    this.leclassementC[x].displayEqui = this.lesequipeskey[this.leclassementC[x].id]
                  }
                  if ( y>0 ) {
                      var sortedclassementC = this.leclassementC.sort((p1, p2) => (p1.rang > p2.rang) ? 1 : (p1.rang < p2.rang) ? -1 : 0)
                      this.leclassementC = sortedclassementC
                  }
